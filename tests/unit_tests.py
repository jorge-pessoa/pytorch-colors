...
class TestTensorConversion(unittest.TestCase):
    def test_no_variable(self):
        a = torch.randn(3, 512, 512).clamp(0, 1)
        a_yuv = colors.rgb_to_yuv(a)
        self.assertEqual(type(a_yuv), torch.FloatTensor)
        a_ = colors.yuv_to_rgb(a_yuv)
        self.assertEqual(type(a_), torch.FloatTensor)

    def test_variable(self):
        a = Variable(torch.randn(3, 512, 512).clamp(0, 1))
        a_yuv = colors.rgb_to_yuv(a)
        self.assertEqual(type(a_yuv), Variable)
        a_ = colors.yuv_to_rgb(a_yuv)
        self.assertEqual(type(a_), Variable)
